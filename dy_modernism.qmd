...
<img src="images/bitly_digyok.png" alt="QR code" height="500px"/><a href="https://bit.ly/digyok"></a><br>
...
-   The underlying data model used by DY offers a robust, scalable solution that has been tested and refined for nearly a decade

-   It is author agnostic and can be modified to meet the needs of any author.
...
# Lighting example: Plot Structure

-   What makes Faulkner's plot structure unique?

# Difficulty and Faulkner

-   Some stories were for a popular audience and some for a "high-brow" audience.
...
fluctuation_total_plot <- plot_ly(
  ...
  hovertemplate = paste('Title: %{x}<extra></extra>')
) 
...
- Very hetergenous corpus of "experimental" and linear narrative.

- Canonical works by Saadat Hassan Manto, Intizar Husain, and Mohinder Singh Sarna
...
partition_chrono <- partition_db %>% 
                   ...
                   select(event_id:chronology, title_of_chapter: gender_author)  %>% 
                    mutate(order_within_page = start_page_event + (page_event_number/100) ) %>% 
...
partition_fluctuation_total <-  partition_fluctuation %>%
  summarise(gender = gender_author, last_name = family_name_author, first_name = given_name_s, flux = sum(total_fluctuation) /
              n()) %>% 
...
# Distribution of Types of Characters
...
::: citation
Veech, Joseph. "cooccur: Probabilistic Species Co-Occurrence Analysis in R." *Journal of Statistical Software*. vol. 69. 2/2016
:::
:::
...
<img src="images/cancerPCA.png" alt="QR code" height="500px"/><a href="https://bit.ly/digyok"></a><br>
